...
//------------------------------------------------------------------------
void PE_Debug :: DumpLineNumber( FILE*fp, DWORD relativeAddress )
  {
...
const char* PE_Debug :: GetSymbolName( PIMAGE_SYMBOL sym )
  ...
  static char buf[ NAME_MAX_LEN ] = { 0 } ;
...
//------------------------------------------------------------------------
void PE_Debug :: DumpSymbolInfo(FILE*fp, DWORD relativeAddress )
  {
  ...
  if( fnSymbol )
    fprintf( fp,  "    function: %s", GetSymbolName( fnSymbol ) ) ;
  else
  ...
  if( fileBase )
    ...
    try
      ...
      if( COFFSymbolCount != 0 && COFFSymbolTable != NULL )
        {
        DumpSymbolInfo( fp, relativeAddress ) ;
        if( COFFDebugInfo )
          DumpLineNumber( fp, relativeAddress ) ;
        }
      else
        {
        fprintf( fp,  " 无调试信息\r\n" ) ;
