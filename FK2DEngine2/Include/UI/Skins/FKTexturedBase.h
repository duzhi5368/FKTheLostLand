/**
*	created:		2013-4-11   22:57
*	filename: 		FKTexturedBase
*	author:			FreeKnight
*	Copyright (C): 	
*	purpose:		
*/
...
#include "FKTexturing.h"
//------------------------------------------------------------------------
namespace FK2DEngine2
{
	namespace Skin
	{
		class TexturedBase : public FK2DEngine2::Skin::Base
		{
		public:

			TexturedBase( FK2DEngine2::video::Base* renderer ) : FK2DEngine2::Skin::Base( renderer )
			{
			}
			...
			virtual void Init( const UITexObjSimpleInfo & TextureName )
			{
				...
				// 按钮
				Colors.Button.Normal					= GetRender()->PixelColour( &m_Texture, 4 + 8 * 2, 508, Color( 0, 255, 255 ) );
				...
				// 标签
				Colors.Tab.Active.Normal				= GetRender()->PixelColour( &m_Texture, 4 + 8 * 4, 508, Color( 0, 255, 255 ) );
				...
				Colors.Tab.Inactive.Normal				= GetRender()->PixelColour( &m_Texture, 4 + 8 * 6, 508, Color( 0, 255, 255 ) );
				...
				// Label
				Colors.Label.Default					= GetRender()->PixelColour( &m_Texture, 4 + 8 * 8, 508, Color( 0, 255, 255 ) );
				...
				// 树状控件
				Colors.Tree.Lines						= GetRender()->PixelColour( &m_Texture, 4 + 8 * 10, 508, Color( 0, 0, 255 ) );
				...
				// 属性
				Colors.Properties.Line_Normal			= GetRender()->PixelColour( &m_Texture, 4 + 8 * 12, 508, Color( 0, 255, 255 ) );
				...
				Colors.Properties.Title					= GetRender()->PixelColour( &m_Texture, 4 + 8 * 13, 500, Color( 0, 255, 255 ) );
				...
				Colors.TooltipText						= GetRender()->PixelColour( &m_Texture, 4 + 8 * 19, 508, Color( 0, 0, 255 ) );
				...
				Colors.Category.Line.Text				= GetRender()->PixelColour( &m_Texture, 4 + 8 * 20, 508, Color( 0, 0, 255 ) );
				...
				Colors.Category.Line.Button				= GetRender()->PixelColour( &m_Texture, 4 + 8 * 21, 500, Color( 0, 255, 255 ) );
				...
				Colors.Category.LineAlt.Text			= GetRender()->PixelColour( &m_Texture, 4 + 8 * 22, 500, Color( 0, 0, 255 ) );
				...
				Colors.Category.LineAlt.Button			= GetRender()->PixelColour( &m_Texture, 4 + 8 * 25, 508, Color( 0, 255, 255 ) );
			...
			{
				const FK2DEngine2::Rect & rect = control->GetRenderBounds();

				if ( bSubmenuOpen || control->IsHovered() )	{ Textures.Menu.Hover.Draw( GetRender(), rect ); }
				if ( bChecked ) { Textures.Menu.Check.Draw( GetRender(), FK2DEngine2::Rect( rect.x + 4, rect.y + 3, 15, 15 ) ); }
			}
			...
			{
				FK2DEngine2::Rect r = control->GetRenderBounds();
				r.x -= 4;
				r.y -= 4;
				r.w += 10;
				r.h += 10;
			...
			virtual void DrawGroupBox( FK2DEngine2::Controls::Base* control, int textStart, int textHeight, int textWidth )
			{
				...
				Textures.GroupBox.Draw( GetRender(), rect, FK2DEngine2::Colors::White, true, false );
				rect.x += textStart + textWidth - 4;
				rect.w -= textStart + textWidth - 4;
				Textures.GroupBox.Draw( GetRender(), rect, FK2DEngine2::Colors::White, false, true, false, false, false, false, false, false, false );
			}
			...
			virtual void DrawActiveTabButton( FK2DEngine2::Controls::Base* control, int dir )
			{
			...
			{
				FK2DEngine2::Rect rect = control->GetRenderBounds();
				GetRender()->SetDrawColor( FK2DEngine2::Color( 255, 100, 255, 255 ) );
			...
			void DrawSliderNotchesH( FK2DEngine2::Rect rect, int numNotches, int dist )
			{
				...
				{
					GetRender()->DrawFilledRect( FK2DEngine2::Rect( rect.x + iSpacing * i, rect.y + dist - 2, 1, 5 ) );
				}
			}

			void DrawSliderNotchesV( FK2DEngine2::Rect rect, int numNotches, int dist )
			{
				if ( numNotches == 0 ) { return; }
				...
				for ( int i = 0; i < numNotches + 1; i++ )
				{
					GetRender()->DrawFilledRect( FK2DEngine2::Rect( rect.x + dist - 1, rect.y + iSpacing * i, 5, 1 ) );
				}
			}
			...
			virtual void DrawSlider( FK2DEngine2::Controls::Base* control, bool bIsHorizontal, int numNotches, int barSize )
			{
				if ( bIsHorizontal )
				{
					...
					rect.y += rect.h * 0.5 - 1;
					rect.h = 1;
					GetRender()->SetDrawColor( FK2DEngine2::Color( 0, 0, 0, 100 ) );
			...
			virtual void DrawComboBox( FK2DEngine2::Controls::Base* control, bool bDown, bool bMenuOpen )
			{
				if ( control->IsDisabled() )
			...
			virtual void DrawComboDownArrow( FK2DEngine2::Controls::Base* control, bool bHovered, bool bDown, bool bMenuOpen, bool bDisabled )
			{
				if ( bDisabled )
			...
			{
				FK2DEngine2::Rect r = FK2DEngine2::Rect( control->GetRenderBounds().x, control->GetRenderBounds().y, 31, 31 );

				if ( bDisabled )	{ return Textures.Window.Close.Draw( GetRender(), r, FK2DEngine2::Color( 255, 255, 255, 50 ) ); }
			...
			virtual void DrawWindowMaximizeButton( FK2DEngine2::Controls::Base* control, bool bDepressed, bool bHovered, bool bDisabled, bool bMaximized )
			{
				...
				if ( !bMaximized )
				{
					if ( bDisabled )	{ return Textures.Window.Maxi.Draw( GetRender(), r, FK2DEngine2::Color( 255, 255, 255, 50 ) ); }
				...
				if ( bDisabled )	{ return Textures.Window.Restore.Draw( GetRender(), r, FK2DEngine2::Color( 255, 255, 255, 50 ) ); }
			...
			virtual void DrawWindowMinimizeButton( FK2DEngine2::Controls::Base* control, bool bDepressed, bool bHovered, bool bDisabled )
			{
				...
				if ( bDisabled )	{ return Textures.Window.Mini.Draw( GetRender(), r, FK2DEngine2::Color( 255, 255, 255, 100 ) ); }
			...
			void DrawTreeNode( Controls::Base* ctrl, bool bOpen, bool bSelected, int iLabelHeight, int iLabelWidth, int iHalfWay, int iLastBranch, bool bIsRoot )
			{
				if ( bSelected )
				{
					Textures.Selection.Draw( GetRender(), FK2DEngine2::Rect( 17, 0, iLabelWidth + 2, iLabelHeight - 1 ) );
				}
