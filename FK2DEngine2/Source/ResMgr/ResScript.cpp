	...
	while (fgets(buf, MAX_PATH, pFile))
	{
		...
		else if (_find_string_no_case(strLine, "dim")) // 属性
		{
				...
				(pos = _find_char(strLine, '\t'))) // 存在类型说明
			{
				...
				{
					FKResTraceColorLn(MY_TRACE_COLOR, L"错误： CResScript::LoadTypeList() 出现异常，无法识别的数据类型:%s !", strType.c_str());
					return FALSE;
				}
				...
				if (pos = _find_char(strLine, ':')) // 指定了域长
				{
					...
					case dt_undefine:
						FKResTraceColorLn(MY_TRACE_COLOR, 
							L"错误： CResScript::LoadTypeList() 出现异常。无效的类型名称 dt_undefine ！");
						return FALSE;
					...
					case dt_byte:
						{
							...
							{
								FKResTraceColorLn(MY_TRACE_COLOR,
									L"错误： CResScript::LoadTypeList() 出现错误，字节过长！");
								return FALSE;
							}
						}
						break;
...
BOOL CResScript::SaveScript(LPCSTR szResScriptFileName, BYTE* pKey)
{
	...
	if (pKey)
	{
		...
		{
			FKResTraceColorLn(MY_TRACE_COLOR, 
				L"错误： CResScript::SaveScript() 出现异常。无法打开文件 %s", file.GetFilePath());
			return FALSE;
		}
...
#ifdef _DEBUG
	char szbuf[256];
	lstrcpyA(szbuf, "释放资源脚本 ['无文件'] 完成。");
	if (m_pFileObject)
		sprintf(szbuf, "释放资源脚本 ['%s'] 完成\r\n", GetScriptFileName());
#endif
...
BOOL CResScript::AddToHashMap(UINT uID, IResObject* pResObject)
{
	...
	if (GetResObject(uID))
	{
		FKResTraceColorLn(MY_TRACE_COLOR, L"ID冲突。ID = %d。", uID);
	}
#endif
...
BOOL CResScript::WriteResObject(IResObject* pResObject, FILE* fp)
{
	...
	for (int num = 0; num < nObjNum; num ++)
	{
		...
		{
			FKResTraceColorLn(MY_TRACE_COLOR, 
				L"错误： CResScript::WriteResObject() 出现异常。对象 %s 的编号为 NO.%d 的子对象不存在！", 
		...
		fwrite(str, lstrlenA(str), 1, fp); // 写"{"
...
BOOL CResScript::WriteBinObject(IResObject* pResObject, vector<ID_OFFSET>& idOffsetTable, int& nFilePos, FILE* fp)
{
	...
	for (int num = 0; num < nObjNum; num ++)
	{
		...
		for(unsigned int k = 0; k < vProp.size(); k++) // 每个属性
		{
			PROPINFO pi = vProp[k];
			...
			else if (pi.IsUnFixProperty()) // 不定长属性
			{
				...
				FKResTraceColorLn(MY_TRACE_COLOR, 
				L"错误： CResScript::WriteBinObject() 出现异常。无法识别的属性！");
		}
...
BOOL CResScript::Dump()
{
	Assert(m_pResObjectRoot);
	DebugMsgLn(L"\r\n------------- 打印整个脚本信息开始 -------------");
	DebugMsgLn(L"【脚本信息】脚本文件 = \"%s\" 对象总数 = %d", this->GetScriptFileName(), this->GetObjectCount());
	m_pResObjectRoot->Dump();
	DebugMsgLn(L"------------- 打印整个脚本信息结束 -------------\r\n");
	return TRUE;
}
#endif
//------------------------------------------------------------------------