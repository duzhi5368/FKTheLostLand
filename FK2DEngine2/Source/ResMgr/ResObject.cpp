...
BOOL CResObject::AddProperty(LPCSTR szPropertyName, LPCSTR szPropertyValue)
{
	if (FindProperty(szPropertyName) >= 0) // exist!
	{
		FKResTraceColorLn(MY_TRACE_COLOR, 
			L"错误： CResObject::AddProperty() 出现异常！属性 %s 不存在！", szPropertyName);
		return FALSE;
	}
...
BOOL CResObject::SetArrayLong (LPCSTR szArrayName, long* lpData, int nCount)
{
	...
	for (int i = 0; i < nCount; i ++)
	{
		...
		*(start - 1) = ',';
		...
		{
			FKResTraceColorLn(MY_TRACE_COLOR, L"错误： CResObject::SetArrayLong() 出现异常。缓冲区过大，需要限制为 %d 字节内", MAX_SCRIPT_LINE_LEN);
			return FALSE;
		}
#endif
	}
...
BOOL CResObject::SetArrayFloat(LPCSTR szArrayName, double* lpData, int nCount)
{
	...
	for (int i = 0; i < nCount; i ++)
	{
		...
		*(start - 1) = ',';
		...
		if (start - buf > MAX_SCRIPT_LINE_LEN)
		{
			FKResTraceColorLn(MY_TRACE_COLOR, L"错误： CResObject::SetArrayFloat() 出现异常：缓冲区过大，需要限制为 %d 字节内", MAX_SCRIPT_LINE_LEN);
			return FALSE;
		}
#endif
	}
...
BOOL CResObject::Dump()
{
	...
	DebugMsgLn(L"【对象信息】 %s : %s = %d", GetObjectName(), GetTypeName(), GetID());
	DebugMsgLn(L"【属性】数目 = %d", GetPropertyCount());
	for (int m=0; m<GetPropertyCount(); m++)
		DebugMsgLn(L"\t%s = %s", GetPropertyName(m), GetPropertyValue(m));
	DebugMsgLn(L"【子对象】数目 = %d", GetSubObjectCount());
	for (int i=0; i<GetSubObjectCount(); i++)
		DebugMsgLn(L"\t『ID - 名字』= %8d,   %s", GetSubObjectByIndex(i)->GetID(),GetSubObjectByIndex(i)->GetObjectName());
