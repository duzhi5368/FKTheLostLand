...
{
		FK_CONTROL_INLINE( OpenToggleButton, Button )
		{
			SetIsToggle( true );
			SetTabable( false );
		}
...
{
		FK_CONTROL_INLINE( TreeNodeText, Button )
		{
...
FK_CONTROL_CONSTRUCTOR( TreeNode )
{
	m_TreeControl = NULL;
...
TreeNode* TreeNode::AddNode( const UITexObjSimpleInfo & strLabel )
{
	TreeNode* node = new TreeNode( this );
	node->SetText( strLabel );
	node->Dock( Pos::Top );
	node->SetRoot( fk_cast<TreeControl> ( this ) != NULL );
...
void TreeNode::SetText( const UITexObjSimpleInfo & text )
{
	m_Title->SetText( text );
}

const UITexObjSimpleInfo & TreeNode::GetText()
{
	return m_Title->GetText();
}

void TreeNode::SetImage( const UITexObjSimpleInfo & text )
{
...
void TreeNode::ExpandAll()
{
	...
	{
		TreeNode* pChild = fk_cast<TreeNode> ( *iter );
