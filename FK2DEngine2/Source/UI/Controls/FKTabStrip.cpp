...
//------------------------------------------------------------------------
FK_CONTROL_CONSTRUCTOR( TabStrip )
{
	m_TabDragControl = NULL;
	m_bAllowReorder = false;
}

bool TabStrip::DragAndDrop_HandleDrop( FK2DEngine2::DragAndDrop::Package* /*pPackage*/, int x, int y )
{
	FK2DEngine2::Point LocalPos = CanvasPosToLocal( FK2DEngine2::Point( x, y ) );
	TabButton* pButton = fk_cast<TabButton> ( DragAndDrop::SourceControl );
	TabControl* pTabControl = fk_cast<TabControl> ( GetParent() );
	...
	if ( DroppedOn )
	{
		FK2DEngine2::Point DropPos = DroppedOn->CanvasPosToLocal( FK2DEngine2::Point( x, y ) );
...
bool TabStrip::DragAndDrop_CanAcceptPackage( FK2DEngine2::DragAndDrop::Package* pPackage )
{
	if ( !m_bAllowReorder )
	{ return false; }
...
{
	FK2DEngine2::Point pLargestTab( 5, 5 );
	int iNum = 0;
...
void TabStrip::DragAndDrop_HoverEnter( FK2DEngine2::DragAndDrop::Package* /*pPackage*/, int /*x*/, int /*y*/ )
{
	if ( m_TabDragControl )
	{
...
void TabStrip::DragAndDrop_HoverLeave( FK2DEngine2::DragAndDrop::Package* /*pPackage*/ )
{
	delete m_TabDragControl;
	m_TabDragControl = NULL;
}

void TabStrip::DragAndDrop_Hover( FK2DEngine2::DragAndDrop::Package* /*pPackage*/, int x, int y )
{
	FK2DEngine2::Point LocalPos = CanvasPosToLocal( FK2DEngine2::Point( x, y ) );
	Base* DroppedOn = GetControlAt( LocalPos.x, LocalPos.y );
	...
	{
		FK2DEngine2::Point DropPos = DroppedOn->CanvasPosToLocal( FK2DEngine2::Point( x, y ) );
		m_TabDragControl->SetBounds( FK2DEngine2::Rect( 0, 0, 3, Height() ) );
