...
//------------------------------------------------------------------------
FK_CONTROL_CONSTRUCTOR( ResizableControl )
{
	m_bResizable = true;
	m_MinimumSize = FK2DEngine2::Point( 5, 5 );
...
void ResizableControl::DisableResizing()
{
	...
	{
		Resizer* resizer = fk_cast<Resizer> ( *it );

		if ( !resizer ) { continue; }

		resizer->SetMouseInputEnabled( false );
		resizer->SetHidden( false );
		SetPadding( Padding( - resizer->Width(), - resizer->Width(),
...
{
	FK2DEngine2::Point minSize = GetMinimumSize();
