...
//------------------------------------------------------------------------
class FK_EXPORT DownArrow : public Controls::Base
{
	public:

		FK_CONTROL_INLINE( DownArrow, Controls::Base )
		{
...
//------------------------------------------------------------------------
FK_CONTROL_CONSTRUCTOR( ComboBox )
{
	SetSize( 100, 20 );
...
void ComboBox::SelectItemByName( const FK2DEngine2::String & name, bool bFireChangeEvents )
{
	...
	{
		MenuItem* pChild = fk_cast<MenuItem> ( *it );
...
FK2DEngine2::Controls::Label* ComboBox::GetSelectedItem()
{
...
void ComboBox::OpenList()
{
	...
	m_Menu->BringToFront();
	FK2DEngine2::Point p = LocalPosToCanvas( FK2DEngine2::Point( 0, 0 ) );
	m_Menu->SetBounds( FK2DEngine2::Rect( p.x, p.y + Height(), Width(), m_Menu->Height() ) );
}
...
void ComboBox::RenderFocus( FK2DEngine2::Skin::Base* /*skin*/ )
{
}
//------------------------------------------------------------------------