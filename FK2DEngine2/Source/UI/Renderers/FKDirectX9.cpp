...
namespace FK2DEngine2
{
	namespace video
	{
		...
		DirectX9::DirectX9( FK2DEngine2::video::IVideoDriver* pDriver )
		{
			m_pD3D = NULL;
			m_iVertNum = 0;
			m_pDriver = pDriver;
			...
			if( m_pDriver != NULL )
			{
				m_pDevice = ((FK2DEngine2::video::CDirect3D9VideoDriver*)m_pDriver)->getHandle();
				((FK2DEngine2::video::CDirect3D9VideoDriver*)m_pDriver)->SetUIVideoDriver( this );
			}
		}
		...
		void DirectX9::DrawTexturedRect( FK2DEngine2::STextureInfo* pTexture, FK2DEngine2::Rect rect, float u1, float v1, float u2, float v2 )
		{
			// LogTrace( pTexture->name.c_str() );
			...
			{
				//if( m_pDriver && !m_pDriver->CurHasTexture() )
				{
					Flush();
		...
		void DirectX9::LoadTexture( FK2DEngine2::STextureInfo* pTexture )
		{
			IDirect3DTexture9* ptr = NULL;
			D3DXIMAGE_INFO ImageInfo;
			HRESULT hr = D3DXCreateTextureFromFileExW( m_pDevice, pTexture->name.GetUnicode().c_str(),
				0, 0, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, &ImageInfo, NULL, &ptr );
		...
		void DirectX9::FreeTexture( FK2DEngine2::STextureInfo* pTexture )
		{
			IDirect3DTexture9* pImage = ( IDirect3DTexture9* ) pTexture->data;
		...
		FK2DEngine2::Color DirectX9::PixelColour( FK2DEngine2::STextureInfo* pTexture, unsigned int x, unsigned int y, const FK2DEngine2::Color & col_default )
		{
			IDirect3DTexture9* pImage = ( IDirect3DTexture9* ) pTexture->data;
		...
		void DirectX9::ClearCurTexture()
		{
			m_pCurrentTexture = NULL;
		}

		bool DirectX9::CurHasTexture()
		{
			if( m_pCurrentTexture == NULL )
			{
				return false;
